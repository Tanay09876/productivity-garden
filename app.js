...
class GameState {
    ...
    plantSeed(x, y, title, seed) {
        ...
        const plant = {
            ...
            y: y,
            plantedAt: Date.now(),
            isGrowing: false,        // ADDED: Track if growth has started
            startTime: null          // ADDED: Track when growth started
        };
    ...
    // ADDED: Method to start plant growth
    startGrowth(plantId) {
        const plant = this.plants.find(p => p.id === plantId);
        if (plant && !plant.isGrowing) {
    ...
    completePlant(plantId) {
        ...
        // ADDED: Check if plant has been started
        if (!plant.isGrowing) {
            showToast('Please start the plant growth first!', 'warning');
            return null;
        }
        
        // MODIFIED: Use startTime instead of plantedAt
    ...
    getGrowthStage(plant) {
        // ADDED: If not growing yet, show as stage 0 (seed)
        if (!plant.isGrowing) {
            return 0;
        }
...
class GardenRenderer {
    ...
    showPlantPopover(plant, x, y) {
        ...
        // MODIFIED: Show/hide buttons based on plant state
        if (plant.isGrowing) {
            // Plant is growing - hide start, show harvest
            ...
            if (completeBtn.disabled) {
                ...
                completeBtn.textContent = `🌸 Harvest (${minutes}m)`;
            } else {
                completeBtn.textContent = '🌸 Harvest Plant';
            }
        } else {
            // Plant not started - show start, hide harvest
            startBtn.style.display = 'flex';
            completeBtn.style.display = 'none';
            startBtn.textContent = '▶️ Start Growing';
        }
...
function bindUIEvents() {
    ...
    // Plant popover - ADDED: Start button listener
    document.getElementById('start-task-btn').addEventListener('click', () => {
        if (gameState.selectedPlant) {
            ...
            if (!plant.isGrowing) {
                if (gameState.startGrowth(plant.id)) {
                    showToast('Plant growth started! 🌱', 'success');
                    hidePopover();
                }
            }
        }
